...
// Pure function — no closures, all params explicit
function drawIsochrones(map, layers, params) {
  const {
...
export default function MapComponent({
  ...
  // Store latest params so the ResizeObserver callback can access them
  const pendingDraw = useRef(null);
  const roRef       = useRef(null);

  // Init map once — never unmounts while App is mounted
  useEffect(() => {
    ...
    // ResizeObserver: fires whenever the map container gets real dimensions
    // This is the reliable trigger — not timers, not rAF guesses
    roRef.current = new ResizeObserver(entries => {
      for (const entry of entries) {
        const { width, height } = entry.contentRect;
        if (width > 0 && height > 0 && pendingDraw.current) {
          map.invalidateSize();
          drawIsochrones(map, layers.current, pendingDraw.current);
          pendingDraw.current = null; // consumed
        }
      }
    });
    roRef.current.observe(mapDivRef.current);
  ...
  // Draw effect — fires on any relevant change
  useEffect(() => {
    ...
    if (!container || container.offsetWidth === 0 || container.offsetHeight === 0) {
      // Container has no size yet — store params, ResizeObserver will draw when ready
      pendingDraw.current = params;
      return;
    }

    // Container has size — draw immediately
